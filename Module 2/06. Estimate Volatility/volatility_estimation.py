...
def estimate_volatility(prices, l):
    ...
    # Compute log returns and square them
    lret = (np.log(prices) - np.log(prices.shift(1)))**2

    # compute exponential moving average for log returns and square root it to get volatility
    volatility = np.sqrt(lret.ewm(alpha=1-l).mean())
